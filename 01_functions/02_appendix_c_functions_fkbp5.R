### Drivers of FKBP5 DNA methylation: additional functions for Appendix C ###
# Author: Vera N. Karlbauer
# Contact: vera_karlbauer@psych.mpg.de
# Date created: 24-09-20204
# Purpose: Source functions for differential DNA methylation analysis with predictor(s) of interest, used in script 12.
...
ses_loop_lm_simple <- function(data, predictor_name){
...
##@ Function 2: ses_loop_lm_additive
...
ses_loop_lm_additive <- function(data, predictor_1, predictor_2){
  # set up results object with CpG names
  ...
  for (i in 1:49){
    ...
    resultsobj[i,6] = rsqpar$partial.rsq[12]
    resultsobj[i,7] = rsqpar$partial.rsq[13]
  }
  # results object as data frame
  resultsobj <- as.data.frame(resultsobj)
  # rename columns in results onjext
...
### Function 3: ses_loop_lm_multiplicative
...
ses_loop_lm_multiplicative <- function(data, predictor_1, predictor_2){
  # set up results object with CpG names
  ...
  for (i in 1:49){
    ...
    resultsobj[i,8] = rsqpar$partial.rsq[13]
    resultsobj[i,9] = rsqpar$partial.rsq[14]
  }
  # results object as data frame
  resultsobj <- as.data.frame(resultsobj)
  # rename columns in results onjext
...
ses_loop_lm_time <- function(data, predictor_name){
  # set up results object with CpG names
